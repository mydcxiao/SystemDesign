...
class Table {
 ...
 private:
   ...
   // dynamic array to store the head (Node *) of each chain
   ListType * hashArray;
   // number of entries present in this table
   int numOfEntries;
   // number of non-empty buckets in this table
   unsigned int numNonemptyBucket;
   
   //Invariant Representatives:
   //-- hashSize > 0
   //-- hashArray size of it equals to hashSize
   //-- numOfEntries >= 0
   //-- numNonemptyBucket >= 0

   //find the longest chain in the hash table
   //return the length of this chain
   unsigned int findLongestChain() const;
};
